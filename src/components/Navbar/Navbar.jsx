...
const Navbar = () => {
  ...
  return (
    <div className="w-full z-[999] relative ">
        <div className="w-full fixed px-8 md:px-14 py-4 font-['Neue Montreal'] flex items-center justify-between">
        <div className="logo">
        ...
        <div className="hidden md:flex  flex-row gap-10 ">
            {
                ["Services", "Our Work", "About Us", "Insights", "ContactUs"].map((item, index) => (
                    <a to={item} key={index} className={`cursor-pointer text-lg font-normal font-['Neue Montreal'] capitalize ${index === 4 && "ml-44"}`}>{item}</a>
                ))
            }
        ...
        {
            showModal && <AnimatePresence>
            <motion.div 
            variants={modalVariants}
            initial="hidden"
            animate="visible"
            exit="exit"
            className="w-full min-h-screen bg-zinc-900 flex flex-col relative">
                        <div className="z-[20] bg-zinc-900 fixed top-0 w-full flex justify-between px-5 pb-20 border border-zinc-500">
                            <div className="logo">
                            ...
                            <div className="absolute top-3 right-5" onClick={() => setShowModal(false)}>
                                <RxCross1 className="text-2xl text-zinc-400"/>
                            </div>
                        </div>
                        <motion.div 
                            ...
                            animate="animate"
                            exit="exit"
                        className="flex flex-col px-4 pt-28 z-[10]">
                            {
                                ["Services", "Our Work", "About Us", "Insights", "ContactUs"].map((item, index) => (
                                    <a to={item} key={index} className={`cursor-pointer text-[11vw] font-semibold  text-zinc-200 font-['Founders_Grotesk'] tracking-tighter capitalize`}>{item}</a>
                                ))
                            }
                        </motion.div>
                        <div className="w-full flex flex-col px-4 pt-8">
                        {
                            ["S:","Instagram", "Behance", "Facebook", "Linkedin"].map((item, index) => {
                                return(
                                    <p className={`text-zinc-200 font-medium font-['Neue_Montreal'] underline underline-offset-2 ${index === 0 && "py-3"} ${index === 0 && "no-underline"} `} key={index}>{item}</p>
                                    )
                                })
                        }
                        </div>
                        <div className="flex px-4 py-8 flex-col gap-1">
                        {
